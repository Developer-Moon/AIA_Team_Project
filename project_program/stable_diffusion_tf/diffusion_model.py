...
class ResBlock(keras.layers.Layer):
    ...
    def call(self, inputs):
        x, emb = inputs     # x : 노이즈 이미지     emb : 타임스텝 
        h = apply_seq(x, self.in_layers)            # x is the input feature map with shape [batch_size, channels, height, width]
        emb_out = apply_seq(emb, self.emb_layers)   # t_emb is the time step embeddings of shape [batch_size, d_t_emb]
...
class SpatialTransformer(keras.layers.Layer):
    def __init__(self, channels, n_heads, d_head):
        ...
        assert channels == n_heads * d_head     # 채널가지고 classification 하니까 이거를 풀디멘션으로 보고 이거 쪼개서 어텐션 수행하는듯
        self.proj_in = PaddedConv2D(n_heads * d_head, 1)
        self.transformer_blocks = [BasicTransformerBlock(channels, n_heads, d_head)]    # 람다인가? 여러번 수행시키는 것 같기는 한데 몇번하냐고
