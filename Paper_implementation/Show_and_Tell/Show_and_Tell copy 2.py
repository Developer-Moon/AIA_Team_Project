...
pickle.dump(features, open(os.path.join('D:\_AIA_Team_Project_Data\Image_Captioning\_paper/paper_features_8k.pkl'), 'wb'))
...
with open(os.path.join('D:\_AIA_Team_Project_Data\Image_Captioning\_data\Flickr8k/captions.txt'), 'r', encoding='UTF-8') as f : # , encoding='UTF-8'
    next(f) # 첫줄 빼고 읽어오기
    captions_doc = f.read()
# print(captions_doc)
...
def clean(mapping): # 맵핑 딕셔너리 안의 caption을 전처리
    for key, captions in mapping.items():
        for i in range(len(captions)):
            ...
            caption = 'startseq ' + " ".join([word for word in caption.split() if len(word) > 1]) + ' endseq'  # if len(word) > 1
            # 스페이스 기준 잘라서 넣기
...
# print('bf_text:', mapping['675153'])

# preprocess the text
clean(mapping)

# after preprocess of text
# print('af_text:', mapping['675153'])
...
split = int(len(image_ids) * 0.80) # train_test_split
...
# save the model
model.save('D:\_AIA_Team_Project_Data\Image_Captioning\_paper/InceptionV3_Flickr8k_Model.h5')


E_Time = time.time() - S_Time    # 46719.95488071442
print('Time :', E_Time) 
'''
...
print('prediction..')
model = load_model('D:\_AIA_Team_Project_Data\Image_Captioning\_paper/InceptionV3_Flickr8k_Model.h5')
...
for idx, image_id in enumerate(test):
  ...
  # split into words
  actual_captions = [caption.split() for caption in caption]
